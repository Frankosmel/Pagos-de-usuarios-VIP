...
from markups import main_menu, months_selection_keyboard, admin_action_keyboard
...
def handle_pay_request(message):
    register_user(message.from_user)
    selected_months[message.from_user.id] = []
    bot.send_message(
        message.chat.id,
        "🗓️ Selecciona los meses que deseas pagar:",
...
def handle_month_toggle(call):
    ...
    sel = selected_months.setdefault(user_id, [])
    if code in sel:
        sel.remove(code)
    else:
        sel.append(code)
    ...
    bot.edit_message_reply_markup(
        ...
        reply_markup=months_selection_keyboard(sel)
    )
    bot.answer_callback_query(call.id, f"Meses seleccionados: {', '.join(sel) or 'ninguno'}")

@bot.callback_query_handler(func=lambda c: c.data == "confirm_payment")
def handle_confirm(call):
    user_id = call.from_user.id
    codes = selected_months.get(user_id, [])
    if not codes:
    ...
    expire = calculate_expiration_date(codes)
    save_payment_request(user_id, codes, amount)

    bot.send_message(
        call.message.chat.id,
        f"💵 Debes pagar <b>{amount} CUP</b> por <b>{total_days} días</b>\n"
        f"Tu membresía vencerá el <b>{expire}</b>.\n"
        "Por favor envía tu comprobante (foto o texto) aquí.",
        parse_mode="HTML",
...
def handle_cancel(call):
    user_id = call.from_user.id
    selected_months.pop(user_id, None)
    bot.send_message(
        call.message.chat.id,
        "❌ Operación cancelada.",
        reply_markup=main_menu()
...
def handle_comprobante(message):
    ...
    # Enviar al grupo staff con botones admin
    if message.photo:
        ...
        bot.send_photo(
            ...
            file_id,
            caption=caption,
            reply_markup=admin_action_keyboard(user_id)
        )
    ...
    bot.send_message(
        message.chat.id,
        "✅ Tu comprobante ha sido enviado para revisión al staff.",
        reply_markup=main_menu()
    )
...
def handle_check_expiration(message):
    ...
    if days_left >= 0:
        bot.send_message(
            message.chat.id,
            f"📅 Tu suscripción vence el <b>{expire_date.strftime('%d/%m/%Y')}</b>.\n"
